...
const Trangchu = () => {
  ...
  useEffect(() => {
    const fetchOpenSubjects = async () => {
      ...
      try {
        const allOpenSubjects = await classService.getAllClasses();
  ...
  const handleRegister = async (selectedClasses) => {
    ...
    try {
      const response = await studentService.dkhpStudent(selectedClasses);
      if (response.code === 1000) {
        ...
        if (listWrong.length > 0) {
          alert(`Đăng ký không thành công cho các lớp: ${listWrong.join(", ")}`);
        }
        if (listProblem.length > 0) {
          alert(`Lỗi gặp phải đối với các lớp: ${listProblem.join(", ")}`);
        }
      } else {
        alert("Đã xảy ra lỗi khi đăng ký học phần. Vui lòng thử lại.");
      }
    } catch (error) {
