...
const OpenSubjectResultModal = ({ isOpen, onClose, result }) => {
  if (!isOpen) return null;
  const { hocKy, nam } = result || {};
      ...
      <div className="bg-white p-6 rounded shadow-lg w-fit">
        <div className="flex justify-between items-center mb-4 gap-4">
          <h2 className="font-semibold text-xl">
            THÊM MÔN HỌC MỞ: Học kỳ {hocKy} - {nam}
          </h2>
          <button
            onClick={onClose}
              ...
              <>
                <div className="flex items-center mb-2">
                  ...
                  <span className="font-medium text-sm ml-1">
                    {result.listTrue.length} Môn học mở thành công
                  </span>
                </div>
                {result.listTrue.map((subject) => (
                  <p key={subject.id} className="text-sm ml-6">
                    Mã: {subject.maMonHoc}
                  </p>
                ))}
              </>
            ...
            {result.listWrong && result.listWrong.length > 0 && (
              <>
                <div className="flex items-center mt-2">
                  ...
                  <span className="font-medium text-sm ml-1">
                    {result.listWrong.length} Môn học mở thất bại
                  </span>
                </div>
                {result.listWrong.map((subject) => (
                  <p key={subject.id} className="text-sm ml-6">
                    Mã: {subject.maMonHoc}
                  </p>
                ))}
...
export default OpenSubjectResultModal;
