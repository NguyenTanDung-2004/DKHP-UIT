    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-500 bg-opacity-75">
      <div className="bg-white p-6 rounded shadow-lg w-96">
        <div className="flex justify-between items-center mb-4">
          <h2 className="font-semibold text-xl">KẾT QUẢ XÓA MÔN MỞ</h2>
          <button
            onClick={onClose}
              ...
              <>
                <div className="flex items-center mb-2">
                  ...
                  <span className="font-medium text-sm ml-1">
                    {result.listTrue.length} Môn học xóa thành công
                  </span>
                </div>
                {result.listTrue.map((subjectId) => (
                  <p key={subjectId} className="text-sm ml-6">
                    {allClasses.find((item) => item.id === subjectId)?.maMonHoc}{" "}
                    : Xóa thành công
                  </p>
                ))}
              </>
            ...
            {result.listWrong && result.listWrong.length > 0 && (
              <>
                <div className="flex items-center mt-2">
                  ...
                  <span className="font-medium text-sm ml-1">
                    {result.listWrong.length} Môn học xóa không thành công
                  </span>
                </div>
                {result.listWrong.map((subjectId, index) => (
                  <p key={subjectId} className="text-sm ml-6">
                    {allClasses.find((item) => item.id === subjectId)?.maMonHoc}{" "}
                    : {problemText(result.listProblem[index])}
                  </p>
                ))}
              </>
            )}
