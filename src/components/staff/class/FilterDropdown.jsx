...
const FilterDropdown = ({ label, options, value, onFilterChange }) => {
  const [isOpen, setIsOpen] = useState(false);
  const [selectedOption, setSelectedOption] = useState(value || label); // Initialize with label if value is null
  ...
  const handleOptionClick = (option) => {
    if (option === "all") {
      setSelectedOption(label); // Display "Tất cả" or the passed label when "All" is selected
      onFilterChange(""); // Pass an empty value to clear the filter
    } else {
    ...
    <div className="relative w-[200px]" onMouseLeave={closeDropdown}>
      ...
      {isOpen && (
        <div className="absolute bg-white border border-gray-300 rounded-lg shadow-lg w-full z-10">
          <ul className="text-sm text-[#2b3034e6] font-normal py-1">
            <li
              key="all"
              className="hover:bg-gray-100 px-4 py-2 cursor-pointer"
              onClick={() => handleOptionClick("all")}
            >
              Tất cả môn học đang mở
            </li>
