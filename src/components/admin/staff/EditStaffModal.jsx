...
const EditStaffModal = ({ isOpen, onClose, staff, onEditStaff }) => {
  ...
  useEffect(() => {
    if (staff) {
      setStaffData({
        ...
        role: staff.flagAdmin ? "admin" : "staff", // set default if role is null
      });
    }
  }, [staff]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    await onEditStaff(staff.id, staffData);
    onClose();
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-500 bg-opacity-75">
      <div className="bg-white p-6 rounded shadow-lg w-[400px]  overflow-y-auto">
        <div className="flex justify-between items-center mb-4">
          <h2 className="font-semibold text-xl">SỬA THÔNG TIN NHÂN VIÊN</h2>
          <button
            onClick={onClose}
